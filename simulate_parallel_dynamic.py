...
if __name__ == '__main__':
    ...
    for num_procs in NUM_PROCS:
        ...
        with Pool(num_procs) as pool:
            all_u = pool.imap(jacobi_partial, arr)
        end = time.time()
    ...
    fig, ax = plt.subplots()
    ax.set_title(f"Multicore with dynamic scheduling runtimes on {N} floorplans", fontsize=14, fontweight="bold", pad=12)
    ax.set_xlabel("p: number of cores", fontsize=11)
    ax.set_ylabel("Wall clock time in seconds", fontsize=11)
    ax.plot([1, 2, 4, 8, 16], times)
    cwd = os.getcwd()
    ...
    fig, ax = plt.subplots()
    ax.set_title(f"Multicore with dynamic scheduling speedups on {N} floorplans", fontsize=14, fontweight="bold", pad=12)
    ax.set_xlabel("p: number of cores", fontsize=11)
    ax.set_ylabel("Speedups: S(p) = T(1)/T(p)", fontsize=11)
    ax.plot([1, 2, 4, 8, 16], speedups)
    cwd = os.getcwd()
